...
test('Cache works for identical parameters', () => {
  ...
  assert(
    time2 < time1 * 0.1,
    `Cache should be much faster: ${time1}ms vs ${time2}ms`,
  );
});
...
test('Rejects invalid country codes', () => {
  ...
  try {
    ...
    threw = true;
    assert(
      e.message.includes('Invalid country codes'),
      'Should mention invalid codes',
    );
  }
...
test('Sanitizes invalid radius values', () => {
  ...
  assert(
    svg.includes('r="0.5"'),
    'Should fallback to default radius for invalid input',
  );
});
...
test('Defaults to circles for invalid shape', () => {
  ...
  assert(
    svg.includes('<circle'),
    'Should fallback to circles for invalid shape',
  );
});
...
test('Handles moderate size maps efficiently', () => {
  ...
  assert(
    elapsed < 1000,
    `50x50 map should be created in <1s, took ${elapsed}ms`,
  );
...
test('SVG output size is reasonable', () => {
  ...
  assert(
    sizeKB < 50,
    `SVG should be <50KB for 10x10 map, was ${sizeKB.toFixed(1)}KB`,
  );
});
